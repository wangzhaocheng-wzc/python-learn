...
那么就可以实现target的Before Relabling中若存在`__keep_this_service__`，且值为`true`的话，则会加入到kubernetes-sd-endpoints这个target中，否则就会被删除。
...
当想要进行服务可控时只需要将下列声明字段写入想要控制服务中就可实现
...
- Prometheus： https://grafana.com/grafana/dashboards/8588 (旧)
...
# 也可以下载离线包进行安装
复制到容器中cp -r devopsprodigy-kubegraf-app /var/lib/grafana/plugins
# 重建pod生效
...
devops插件安装后，设置密钥
cat ~/.kube/config 和 /etc/kubernetes/pki中
